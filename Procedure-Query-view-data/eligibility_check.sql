...
BEGIN
    ...
    LEFT JOIN (
        SELECT a.Std_Reg, a.Course_ID, SUM(a.No_hour_Attended) AS hours_attended, c.No_of_hours AS total_possible_hours,
               ROUND(SUM(a.No_hour_Attended)/c.No_of_hours*100,2) AS percentage
        FROM Attendance a
        JOIN Course c ON c.Course_ID = a.Course_ID
        WHERE a.Course_ID = in_course_id
        GROUP BY a.Std_Reg, a.Course_ID, c.No_of_hours
    ) att ON att.Std_Reg = s.Std_Reg AND att.Course_ID = in_course_id
    LEFT JOIN (
